import { CollateralUpdateEvent, SettleEvent, TradeEvent } from '..';
...
export default async function fetchPositionEventDataByHash(lyra, transactionHashOrReceipt) {
    const receipt = typeof transactionHashOrReceipt === 'string'
    ...
    if (tradeEvents.length) {
        ...
        const marketContractAddresses = contractAddresses.find(marketAddresses => marketAddresses.optionToken === optionTokenAddress);
    ...
    const eventsByPositionID = (await fetchPositionEventDataByIDs(lyra, positionIds.map(positionId => ({ positionId, marketAddress }))))[marketAddress];
    const events = Object.values(eventsByPositionID).map(({ trades, collateralUpdates, transfers, settle: _settle }) => {
        ...
        const settleData = (_settle === null || _settle === void 0 ? void 0 : _settle.transactionHash) === transactionHash ? _settle : null;
