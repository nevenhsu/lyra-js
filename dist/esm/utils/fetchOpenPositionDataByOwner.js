...
export default async function fetchOpenPositionDataByOwner(lyra, owner) {
    ...
    const positionIds = positionsByMarketAddress.flatMap(({ market, positions }) => positions.map(position => ({ positionId: position.positionId.toNumber(), marketAddress: market })));
    ...
    return positionsByMarketAddress.flatMap(({ positions: openPositionStructs, market: marketAddress }) => {
        return openPositionStructs.map(openPositionStruct => {
            ...
            return getOpenPositionDataFromStruct(owner, openPositionStruct, option, trades, collateralUpdates, transfers, settle);
        });
    });
}
