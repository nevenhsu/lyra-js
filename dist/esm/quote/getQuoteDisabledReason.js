...
export default function getQuoteDisabledReason(option, spotPrice, size, premium, newIv, newSkew, newBaseIv, isBuy, isForceClose, priceType, isOpen, network) {
    ...
    const callDelta = toBigNumber(getDelta(timeToExpiryAnnualized, fromBigNumber(newIv), fromBigNumber(spotPrice), fromBigNumber(strikePrice), fromBigNumber(rate), true));
    ...
    if (hedgerView &&
        ...
        !canHedge(getQuoteSpotPrice(market, priceType), market.params.netDelta, option, size, increasesPoolDelta, hedgerView, poolHedgerParams, network)) {
    ...
    if (market.lyra.network === Network.Arbitrum) {
        ...
        const gmxAdapterView = adapterView;
