...
export class Admin {
    ...
    async setAdapterMarketPricingParams(marketAddressOrName, params) {
        ...
        const owner = await exchangeAdapter.owner();
        const calldata = exchangeAdapter.interface.encodeFunctionData('setMarketPricingParams', [
            market.address,
            toParams,
        ]);
        const tx = buildTx(this.lyra.provider, this.lyra.provider.network.chainId, exchangeAdapter.address, owner, calldata);
    ...
    async setAdapterMarketConfigurationParams(marketAddressOrName, params) {
        const market = await this.lyra.market(marketAddressOrName);
        if (!market.params.adapterView) {
            throw new Error('Adapter market configuration parameters not supported on this market');
        }
        const marketAdapterConfig = market.params.adapterView.config;
        const fromParamsFlat = {
        ...
        const exchangeAdapter = getLyraContract(this.lyra, Version.Newport, LyraContractId.ExchangeAdapter);
        const owner = await exchangeAdapter.owner();
        const calldata = exchangeAdapter.interface.encodeFunctionData('setMarketAdapterConfiguration', [
            market.address,
    ...
    async setFuturesPoolHedgerParams(marketAddressOrName, params) {
        ...
        const calldata = market.lyra.network === Network.Optimism
            ? futuresPoolHedger.interface.encodeFunctionData('setFuturesPoolHedgerParams', [toParams])
            : futuresPoolHedger.interface.encodeFunctionData('setFuturesPoolHedgerParams', [toParams]);
        const tx = buildTx(this.lyra.provider, this.lyra.provider.network.chainId, futuresPoolHedger.address, market.params.owner, calldata);
        tx.gasLimit = BigNumber.from(10000000);
        return { params: toParams, tx };
    }
